...
class ThemeManager<T extends Boolean> {
    ...
    public static void applyTheme(Scene scene) {
        if (isDarkMode) {
            scene.setFill(javafx.scene.paint.Color.DARKSLATEGRAY);
            applyToAllRegions(scene, javafx.scene.paint.Color.WHITE, javafx.scene.paint.Color.BLACK);
        } else {
            scene.setFill(javafx.scene.paint.Color.LIGHTGRAY);
            applyToAllRegions(scene, javafx.scene.paint.Color.BLACK, javafx.scene.paint.Color.WHITE);
        }
    }

    private static void applyToAllRegions(Scene scene, javafx.scene.paint.Color textColor, javafx.scene.paint.Color backgroundColor) {
        for (var node : scene.getRoot().getChildrenUnmodifiable()) {
            if (node instanceof Region region) {
                region.setStyle("-fx-background-color: " + toHex(backgroundColor) + "; -fx-text-fill: " + toHex(textColor) + ";");
            }
        }
    }
